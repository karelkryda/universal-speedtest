...
class Ookla {
        ...
        try {
            ...
            return new Promise(resolve => {
                servers.forEach(server => {
                    ...
                    if (distanceUnit === index_js_1.DistanceUnits.km) {
                        server.distance = (0, index_js_2.convertMilesToKilometers)(server.distance);
                    }
                    // Measure server latency in parallel
                    testsInProgress++;
                    const socketClient = (0, index_js_2.createSocketClient)(server.host);
    ...
    async measureUploadSpeed(bestServer, testUUID) {
        ...
        const openServerConnection = () => {
            ...
            (0, index_js_2.createPostRequest)(uploadUrl, dataStream, abortSignal).catch(decreaseConnections);
        };
