...
export class UniversalSpeedTest {
     ...
     */
    public listOoklaServers(serversToFetch?: number): Promise<OAServer[]> {
     ...
     * @returns {Promise<OAServer[]>} Ookla test servers
     */
    public searchOoklaServers(searchTerm: string, serversToFetch?: number): Promise<OAServer[]> {
        const ooklaTest = new Ookla(this.options);
     ...
     */
    public performOoklaTest(server?: OAServer): Promise<OAResult> {
        const ooklaTest = new Ookla(this.options);
